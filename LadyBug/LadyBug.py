...
def start_automatic(btn2, n, freq, res_x, res_y, col_ini, col_fin):  # n = number of hits, freq = frequency
    if "btn-nclicks-3" == ctx.triggered_id:
        ...
        i=col_ini*res_y
        print("Número de puntos de inspección: "+ str((col_fin+1)*res_y-i))

        if i > (res_x*res_y):
            msg="Número de columna inicial superada\n"
            print(msg)
            return html.Div(msg)

        if ((col_fin+1)*res_y) > (res_x*res_y):
            msg="Número de columna final superada\n"
            print(msg)
            return html.Div(msg)
